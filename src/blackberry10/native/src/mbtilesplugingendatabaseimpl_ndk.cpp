...
namespace webworks {
	...
	MBTilesPluginGenDataBaseImplNDK::MBTilesPluginGenDataBaseImplNDK(MBTilesPluginJS *parent)
	: MBTilesPluginGenImplNDK(parent)
	{
		database = NULL;
	}

	MBTilesPluginGenDataBaseImplNDK::~MBTilesPluginGenDataBaseImplNDK()
	{
		close();
	}

	// The extension methods are defined here
	Json::Value MBTilesPluginGenDataBaseImplNDK::open(const std::string& callbackId, const std::string name)
	{
		//
		close();
		...
		if (dir != NULL) {
			...
			dir += QString::fromStdString(name);
			QFile* path = new QFile(dir);
	...
	void MBTilesPluginGenDataBaseImplNDK::close()
	{
		if (database != NULL) {
	...
	bool MBTilesPluginGenDataBaseImplNDK::isOpen()
	{
		return database != NULL;
	}

	Json::Value MBTilesPluginGenDataBaseImplNDK::getMetaData(const std::string& callbackId)
	{
	...
	Json::Value MBTilesPluginGenDataBaseImplNDK::getMinZoom(const std::string& callbackId)
	{
		Json::FastWriter writer;
		Json::Value root;
	...
	int MBTilesPluginGenDataBaseImplNDK::getMaxZoomValue()
	{
		int max = -1;
		 // test if the db is open
		if(isOpen() == true) {
	...
	Json::Value MBTilesPluginGenDataBaseImplNDK::getMaxZoom(const std::string& callbackId)
	{
		Json::FastWriter writer;
		Json::Value root;
	...
	Json::Value MBTilesPluginGenDataBaseImplNDK::getTile(const std::string& callbackId, int zoom_level, int column, int row)
	{
		Json::FastWriter writer;
		Json::Value root;
	...
	Json::Value MBTilesPluginGenDataBaseImplNDK::getExecuteStatement(const std::string& callbackId, const std::string query, const QList<Json::Value> params)
	{
		Json::FastWriter writer;
		Json::Value root;
		// test if the database is open
		if (isOpen() == true) {
			...
			if( ret == SQLITE_OK) {
				...
				if (result == true) {
					...
					root[KEY_EXECUTE_STATEMENT] = array;
	...
	Json::Value MBTilesPluginGenDataBaseImplNDK::getDirectoryWorking(const std::string& callbackId)
	{
		Json::Value root;
		QString dir = getDirectory();
		if (dir != NULL) {
	...
	bool MBTilesPluginGenDataBaseImplNDK::bindValue(sqlite3_stmt* stmt, QList<Json::Value> params) {
